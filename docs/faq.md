...
#### 如果网络运营商或政府屏蔽了 Tor ，如何使它工作？

ZeroNet 目前未包含 [Tor 可插拔传输](https://www.torproject.org/docs/pluggable-transports.html.en) 。使用它最简单的方法是打开 Tor Browser，配置可拔插传输，然后通过使用 `--tor_controller 127.0.0.1:9151 --tor_proxy 127.0.0.1:9150` 参数或添加它到 `zeronet.conf` 更改 ZeroNet 的配置来使用浏览器的 Tor 客户端。
...
可以, 你需要复制 `data/users.json` 到你的新机器。
...
使用 [虚荣地址生成器](https://bitcointalk.org/index.php?topic=25804.0) 来生成一个。当你有了你的密钥对后，创建 `data/1YourPublicKey...tCkBzAXTKvJk4uj8` 目录。放置一些文件。

然后访问 [http://127.0.0.1:43110/1YourPublicKey...tCkBzAXTKvJk4uj8/](http://127.0.0.1:43110/1YourPublicKey...tCkBzAXTKvJk4uj8/) 。 把 “0” 按钮拉到左边并使用侧边栏签名你的站点。
...
> __提示:__ 为使得 ZeroHello 链接到你的域名而不是你的比特币地址，添加一个域名键值到你的 content.json。([例子](https://github.com/HelloZeroNet/ZeroBlog/blob/master/content.json#L6))
